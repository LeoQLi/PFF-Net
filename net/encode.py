...
from .base import knn_group, get_knn_idx, Conv1D
...
class GraphConv(nn.Module):
    ...
    def get_edge_feature(self, x, pos, knn_idx):
        ...
        return: (B, C, N, K)
        """
        knn_feat = knn_group(x, knn_idx)                  # (B, C, N, K)
    ...
    def forward(self, x, pos, knn_idx):
        ...
        if self.knn > 1:
            ...
            edge_feat = edge_feat.view(B, -1, N * self.knn)                             # (B, C, N*K)
            x = x.unsqueeze(-1).repeat(1, 1, 1, self.knn).view(B, C, N * self.knn)      # (B, C, N*K)
        else:
            edge_feat = x
